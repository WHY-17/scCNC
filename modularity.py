...
class CapsuleNetwork(nn.Module):
    def __init__(self,
        ...
        self.encode = nn.Linear(self.n_z, 10)
        ...
        self.mu = Parameter(torch.Tensor(n_clusters, self.n_z))
    ...
    def get_z(self, x):
        digit = self.digits(self.primary(self.conv1(x)))
        z = digit.reshape(len(digit), self.n_z)
        embedding = self.encode(z)
        return embedding
